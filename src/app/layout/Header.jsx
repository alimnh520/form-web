...
const Header = () => {
    ...
    return (
        <div className={`w-full sticky top-0 transition-all duration-300 ${scroll > 5 ? 'h-10' : 'h-16'} flex items-center justify-center bg-transparent scroll-smooth z-50 backdrop-blur-md text-black border-b border-b-gray-500 sm:h-auto sm:relative`}>
            ...
            <nav className='w-full px-20 h-full flex items-center justify-between sm:px-10 sm:mt-5 sm:flex-col sm:h-fit sm:items-start'>
                ...
                <ul className={`h-full flex items-center justify-center space-x-5 sm:flex-col sm:space-x-0 sm:space-y-4 sm:items-start sm:mt-5 sm:overflow-hidden transition-all duration-300 ${showNav ? 'sm:h-[166px]' : 'sm:h-0'}`}>
                    ...
                    <li className={`${linkCls} ${pathName == '/' ? 'before:w-full text-red-500':'before:w-0'}`} onClick={() => setShowNav(!showNav)}><Link href="/">Forms</Link></li>
