...
const page = () => {
    ...
    return (
        <div className='w-full h-auto flex flex-col items-center justify-start p-5 gap-y-12 sm:gap-y-5'>
            <h1 className='text-4xl font-bold border-b border-b-gray-400 py-5 sm:text-2xl sm:py-3'>ভূমি রেকর্ড ও ম্যাপ</h1>
                    ...
                    user ? user.map((elem) => {
                        return (
                            <div className="flex items-center justify-center gap-x-5 sm:gap-x-3" key={elem._id}>
                                ...
                                <Link href={`/dashboard/online-service/mutation/${elem._id}`} className="px-5 py-1 bg-blue-600 text-lg rounded-lg font-semibold text-white sm:text-sm sm:px-2 sm:py-2">
                                    See details
                                </Link>
