...
const page = () => {
    ...
    const handleDelete = async () => {
        setLoading(true);
        try {
            const response = await fetch('/api/delete/deleta-tax/land-tax1', {
                method: 'POST',
                headers: {
    ...
    return (
        <div className='w-full h-auto flex flex-col items-center justify-start p-5 gap-y-12'>
            <div href="/components/land-tax3" className="shadow-[0_0_10px_rgba(0,0,0,0.5)] cursor-pointer bg-white rounded-md flex flex-col items-center justify-center relative space-y-5 sm:h-52 h-56 w-96">
                    ...
                    user ? user.map((elem) => {
                        return (
                            <div className="flex items-center justify-center gap-x-5" key={elem._id}>
                                ...
                                <Link href={`/dashboard/online-service/mutation/${elem._id}`} className="px-5 py-1 bg-blue-600 text-lg rounded-lg font-semibold text-white">
                                    See details
                                </Link>
