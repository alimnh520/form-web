...
const page = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch("/api/post/submit-landtax/landTex3", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <div className="w-full h-screen flex flex-col items-center bg-white text-black justify-center relative px-20 sm:px-10 sm:justify-start sm:py-5 sm:h-auto">
      {submit && (
        <p className=" absolute text-lg top-1/2 -translate-y-1/2 bg-red-500 px-10 py-2 text-white rounded-md z-20">
          আবেদনটি জমা হয়েছে
        </p>
      )}
      ...
      <form
        ...
        className="w-10/12 sm:w-full space-y-6 gap-x-7 grid grid-cols-4 items-center justify-center mt-5 sm:grid-cols-1 sm:grid-rows-none relative"
      >
      ...
      <button
        className="bg-blue-600 mt-10 px-10 py-2 text-lg text-white rounded-md "
        onClick={() => router.back()}
      >
        Back
      </button>
