...
export const MouzaMap = () => {
    ...
    const [sitNumber, setSitNumber] = useState('');
    ...
    const mouzaSubmit = async (e) => {
        setLoading(true);
        const res = await fetch('/api/user/submit-data/mouza', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ divisionName, districtName, upazilaName, mouzaName, sitNumber, email: user.email, username: user.username }),
        });
        setLoading(false);
    ...
    return (
        <div className="w-full h-full flex flex-col items-center p-7">
            ...
            <h1 className='text-4xl text-center w-full font-bold border-b border-b-gray-400 py-5'>মৌজা ম্যাপ</h1>
            ...
            <div className='w-10/12 space-y-6 gap-x-7 grid grid-cols-4 items-center justify-center mt-5 relative'>
                ...
                <div className='flex flex-col items-start w-full relative py-4 h-12 border border-green-500 rounded-md'>
                    <p className=' absolute -top-3 rounded-md left-3 text-sm backdrop-blur-md px-2 bg-white text-green-700'>সিট নাম্বার <span className='text-red-500 relative top-1 text-lg '>*</span></p>
                    <div name="" id="" className='bg-transparent w-full relative outline-none'>
                        <input type="text" className='outline-none w-full placeholder:text-sm placeholder:text-neutral-600 text-neutral-600 border-b-green-500 px-4' value={sitNumber} placeholder='সিট নাম্বার লিখুন...'
                            onChange={(e) => setSitNumber(e.target.value)} />
                    </div>
                </div>
            ...
            <div className="w-full h-auto flex flex-col items-center mt-10 gap-y-5">
                ...
                <div className="w-full h-auto flex flex-col">
                    <div className="w-full grid grid-cols-8 bg-green-600 text-white font-bold">
                        ...
                        <p className="text-center border-r border-b py-3">সিট নাম্বার</p>
                        <p className="text-center border-r border-b py-3">স্টাটাস</p>
                        <p className="text-center border-r border-b py-3">অ্যাকশন</p>
                    </div>
                    {
                        mouzaData ? (
                            mouzaData.slice().reverse().filter((currElm) => currElm.email === user.email).map((elem, index) => {
                                return (
                                    <div className="w-full flex flex-col" key={elem._id}>
                                        <div className="w-full grid grid-cols-8">
                                            ...
                                            <p className="text-center border-r border-b py-3 overflow-x-scroll">{elem.sitNumber}</p>
