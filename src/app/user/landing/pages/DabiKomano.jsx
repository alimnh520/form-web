...
export const DabiKomano = ({ getNewMoney }) => {
    ...
    const [dabiKomanoData, setDabiKomanoData] = useState('');
    ...
    useEffect(() => {
        ...
        const dabiKomanoData = async () => {
            try {
                const response = await fetch("/api/user/submit-data/dabikomano", {
                    method: "GET",
                });
                const data = await response.json();
                setDabiKomanoData(data.message);
            } catch (err) {
                console.log(err);
            }
        };
    ...
    const SelfLandTaxSubmit = async (e) => {
        ...
        setLoading(true);
        const res = await fetch('/api/user/submit-data/dabikomano', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify({ divisionName, districtName, amount, upazilaName, mouzaName, khatianName, email: user.email, username: user.username }),
        });
        setLoading(false);
        ...
        <div className="w-full h-full flex flex-col items-center p-7">
            ...
            {takaKata &&
                <div className="max-w-md mx-auto absolute top-40 mt-10 p-6 bg-yellow-100 border-l-4 border-yellow-500 text-yellow-800 rounded-lg shadow-md z-20">
                    <h2 className="text-xl font-semibold mb-2">⚠️ গুরুত্বপূর্ণ নির্দেশনা</h2>
                    <p className="text-base leading-relaxed">
                        আপনার একাউন্ট থেকে <span className="font-bold text-red-600">{convertTaka((30 * amount) / 100)} টাকা</span> কেটে নেওয়া হবে।
                    </p>
            ...
            <div className='w-10/12 space-y-6 gap-x-7 grid grid-cols-4 items-center justify-center mt-5 relative'>
                ...
                <div className='flex flex-col items-start w-full relative py-4 h-12 border border-green-500 rounded-md'>
                    <p className=' absolute -top-3 rounded-md left-3 text-sm backdrop-blur-md px-2 bg-white text-green-700'>টাকার পরিমান <span className='text-red-500 relative top-1 text-lg '>*</span></p>
                    <div name="" id="" className='bg-transparent w-full relative outline-none'>
                        <input type="text" className='outline-none w-full placeholder:text-sm placeholder:text-neutral-600 text-neutral-600 border-b-green-500 px-4' value={amount} placeholder='টাকার পরিমান লিখুন...'
                            onChange={(e) => setAmount(e.target.value)} />
                    </div>
                </div>

                <div className='flex flex-col items-start w-full relative py-4 h-12 border border-green-500 rounded-md'>
                    <p className=' absolute -top-3 rounded-md left-3 text-sm backdrop-blur-md px-2 bg-white text-green-700'>কমানো দাবির পরিমান <span className='text-red-500 relative top-1 text-lg '>*</span></p>
                    <div name="" id="" className='bg-transparent w-full relative outline-none'>
                        <input type="text" disabled className='outline-none w-full placeholder:text-xl placeholder:text-neutral-600 text-neutral-600 border-b-green-500 px-4' value={(30 * amount) / 100} />
                    </div>
                </div>
            ...
            <div className="w-full h-auto flex flex-col items-center mt-10 gap-y-5">
                ...
                <div className="w-full h-auto flex flex-col">
                    ...
                    {
                        dabiKomanoData ? (
                            dabiKomanoData.slice().reverse().filter((currElm) => currElm.email === user.email).map((elem, index) => {
                                return (
                                    <div className="w-full flex flex-col" key={elem._id}>
                                        <div className="w-full grid grid-cols-9">
                                            ...
                                            <p className="text-center border-r border-b py-3 overflow-x-scroll">{elem.amount && convertTaka(elem.amount.toString())}</p>
